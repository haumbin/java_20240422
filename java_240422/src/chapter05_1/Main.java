...
//		p2.swim(); // 업캐스팅은 슈퍼클래스의 멤버만 접근가능(오류 발생)
		p2.walk();
...
//		new Swim2(); //인터페이스는 객체 생성 못한다.
		...
		Swim2[] s2= new Swim2[2]; // 인터페이스가 인터페이스 배열 2 개를 가리킨다.뭔소리지?
		// 객체 배열과 형태가 비슷
		//s2[0],s2[1]
...
//		s2[2]=new Penguin(); //컴파일러 단계는 알수 없고, 런타임 실행시 오류가 발생한다.
